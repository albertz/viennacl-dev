...
namespace viennacl{
  ...
  namespace generator{
    ...
    namespace detail{
      ...
      void statement_representation(scheduler::statement const & statement, void* (&memory)[64], unsigned int & current_arg, char*& ptr){
          ...
          for(std::size_t i = 0 ; i < expr.size() ; ++i){
            ...
            if(node.lhs_type_family_!=COMPOSITE_OPERATION_FAMILY){
              utils::call_on_element(node.lhs_type_family_, node.lhs_type_, node.lhs_, representation_functor(memory, current_arg, ptr));
            }

            {
              const char * op_expr = detail::generate(node.op_type_);
            ...
            if(node.rhs_type_family_!=COMPOSITE_OPERATION_FAMILY){
              utils::call_on_element(node.rhs_type_family_, node.rhs_type_, node.rhs_, representation_functor(memory, current_arg, ptr));
            }
